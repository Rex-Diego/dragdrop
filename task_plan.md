...
- [x] 覆盖八方向笔尖平移、侧键缩放、途中按键不切换、原生连接及取消；lint/typecheck/197 tests/build/语法与差异检查通过。
...
- [x] 同文件、跨文件、Canvas 各自成组；动作说明只描述本动作，相关尺寸、文件位置、别名等跟随各自作用域。
- [x] 根据本机 Canvas 核心鼠标类型和左键条件适配笔事件，复用原生连线、八方向缩放及取消状态机。
- [x] 覆盖连接点优先命中、SVG 空白区域不误拦截、控件移除后事件转发、鼠标/输入框旁路与侧键开关边界。
- [x] lint、typecheck、25 files / 189 tests、生产 build 通过。
- [x] 两个实际插件目录的六文件部署完成，哈希一致，保留用户配置与 graph-worker.js。
- [ ] 重载后 Surface Pen 验收：连接已有卡片、新卡片菜单、四边四角缩放、取消、普通区域平移、鼠标与跨弹窗拖放。
...
- [x] 新增独立的文字选区菜单秒数设定：Markdown、PDF/PDF++ 与 Canvas 卡片编辑器选区均支持 `-1` 保留原生菜单、`0` 不显示本次选区菜单、正数秒数（允许小数）显示半透明且避让选区的菜单，并在未 hover 时按秒数自动关闭；不复用块操作菜单开关
- [x] 为文字选区菜单补设置迁移、中英文文案、owner-document 生命周期、hover 暂停/离开重计时、菜单定位与关闭行为测试
- [ ] 在 Obsidian 实机复核 Markdown、PDF++、Canvas 卡片编辑器和 Surface 选区菜单：`-1/0/正值`、半透明、避让选区、hover 暂停、离开重计时、主窗口与 Popout 清理
